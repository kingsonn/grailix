...
export default function PredictClient() {
  ...
  const fetchNextPrediction = async () => {
    ...
    setIsLoading(true);
    setError(null);
    // Reset betting closed state when fetching new prediction
  ...
  // Reset betting closed state when prediction changes
  useEffect(() => {
    if (prediction) {
      // Check if betting is actually closed based on timestamp
  ...
  }, [prediction?.id]); // Only run when prediction ID changes
          ...
          <>
                ...
                <div className="text-right">
                  ...
                  {prediction.betting_close && !bettingClosed && (
                    <div className="text-yellow-400 font-bold text-sm flex items-center gap-1 justify-end">
                      <span>🔒</span>
                      <span>Betting closes: {timeToBettingClose || "Loading..."}</span>
                    </div>
                  )}
                  {bettingClosed && (
            ...
            {process.env.NODE_ENV === "development" && prediction && (
              <div className="bg-gray-900 border border-yellow-600 rounded-lg p-4 mt-4">
                ...
                <button
                  onClick={async () => {
                    if (!confirm(`Force expire prediction ${prediction.id}?`)) return;
                    try {
                      const response = await fetch("/api/dev/force-expire", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                      ...
                      const data = await response.json();
                      if (data.success) {
                        alert(`✅ Prediction ${prediction.id} force-expired!`);
                        fetchNextPrediction();
                      } else {
                  ...
                  className="w-full bg-gray-700 hover:bg-gray-600 text-yellow-400 font-bold py-2 px-4 rounded-lg transition-colors text-sm flex items-center justify-center gap-2"
                >
                  <span>⚡</span>
                  <span>Force Expire This Prediction</span>
                </button>
        ...
        {showStakeModal && selectedPosition && (
          <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full">
              ...
              <p className="text-gray-400 mb-4">Your Balance: {user?.real_credits_balance} credits</p>
                  ...
                  <button
                    ...
                    className={`py-3 rounded-lg font-bold transition-colors ${
                      stakeAmount === amount
                        ? "bg-blue-600 text-white"
                        : "bg-gray-700 text-gray-300 hover:bg-gray-600"
                    } ${user && user.real_credits_balance < amount ? "opacity-50 cursor-not-allowed" : ""}`}
                  >
                    {amount}
                  </button>
                ))}
